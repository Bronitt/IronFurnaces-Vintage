...
import com.xenomustache.ironfurnaces.blocks.BlockIronFurnace;
...
ISidedInventory {
    ...
    public String getName() {
        return this.hasCustomName() ? this.furnaceCustomName : "container.furnace_0";
    }
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        this.currentItemBurnTime = TileEntityIronFurnace.getItemBurnTime(this.furnaceItemStacks.get(1));
        ...
        if (!this.world.isRemote) {
            ...
            if (this.isBurning() || !itemstack.isEmpty() && !this.furnaceItemStacks.get(0).isEmpty()) {
                if (!this.isBurning() && this.canSmelt()) {
                    this.currentItemBurnTime = this.furnaceBurnTime = TileEntityIronFurnace.getItemBurnTime(itemstack);
                    if (this.isBurning()) {
                        flag1 = true;
            ...
            if (flag != this.isBurning()) {
                ...
                keepInventory = true;
                if (flag1) {
                    this.world.setBlockState(this.pos, IFBlocks.ironFurnaceActive.getDefaultState().withProperty(BlockIronFurnace.FACING, iblockstate.getValue(BlockIronFurnace.FACING)), 3);
                    this.world.setBlockState(this.pos, IFBlocks.ironFurnaceActive.getDefaultState().withProperty(BlockIronFurnace.FACING, iblockstate.getValue(BlockIronFurnace.FACING)), 3);
                } else {
                    this.world.setBlockState(this.pos, IFBlocks.ironFurnaceIdle.getDefaultState().withProperty(BlockIronFurnace.FACING, iblockstate.getValue(BlockIronFurnace.FACING)), 3);
                    this.world.setBlockState(this.pos, IFBlocks.ironFurnaceIdle.getDefaultState().withProperty(BlockIronFurnace.FACING, iblockstate.getValue(BlockIronFurnace.FACING)), 3);
                }
                keepInventory = false;
                if (tileentity != null) {
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        ...
        ItemStack itemstack = this.furnaceItemStacks.get(1);
        return TileEntityIronFurnace.isItemFuel(stack) || SlotFurnaceFuel.isBucket(stack) && itemstack.getItem() != Items.BUCKET;
    }
